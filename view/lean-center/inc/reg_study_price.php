...
<div id="content">
	<section id="business_reg">
		...
		<?php if(isset($_SESSION['auth']) && !isset($_SESSION['auth']['user'])) {
			...
			unset($_SESSION['auth']);} ?>
		<h3>Регистрация участников для ознакомления с ценой обучения</h3>
		<form method="post" action="" id="reg_form">
			<table class="zavtrak">
				...
				<tr>
					<td><span style="color:#c00; font-size:20px;">*</span></td>
					<td><input type="text" name="reg_quantity" id="reg_quantity" value="<?=$content['reg_quantity']?>" placeholder="Количество слушателей"></td>
				</tr>
			</table>
			<p class="small" style="color:#f77; font-size:12px;">Поля, помеченные * являются обязательными к заполнению.</p>
			<p class="small">Указанные вами реквизиты будут использованы исключительно для информирования Вас о стоимости обучения, но не для рассылки спама!!!</p>
			<input type="submit" name="reg_study_price" id="reg_business" value="Запросить стоимость обучения">
		</form>
	</section> <!-- #reg -->
</div> <!-- #content_reg -->