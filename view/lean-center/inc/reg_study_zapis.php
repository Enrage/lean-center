...
<div id="content">
	<section id="business_reg">
		...
		<?php if(isset($_SESSION['auth']) && !isset($_SESSION['auth']['user'])) {
			...
			unset($_SESSION['auth']);} ?>
		<h3>Регистрация участников на обучение</h3>
		<form method="post" action="" id="reg_form">
			...
			<p class="small">Указанные вами реквизиты будут использованы исключительно для записи на обучение, но не для рассылки спама!!!</p>
			<input type="submit" name="reg_study_zapis" id="reg_business" value="Записаться на обучение">
		</form>
	</section> <!-- #reg -->
</div> <!-- #content_reg -->